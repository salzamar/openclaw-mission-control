...
const AgentsSidebar: React.FC<AgentsSidebarProps> = ({
	...
	if (agents === undefined) {
		return (
			<aside
				className={`[grid-area:left-sidebar] sidebar-drawer sidebar-drawer--left bg-white border-r border-border flex flex-col overflow-hidden animate-pulse ${isOpen ? "is-open" : ""} ${isCollapsed ? "collapsed" : ""}`}
				aria-label="Agents"
			>
	...
	return (
		<aside
			className={`[grid-area:left-sidebar] sidebar-drawer sidebar-drawer--left bg-white border-r border-border flex flex-col overflow-hidden ${isOpen ? "is-open" : ""} ${isCollapsed ? "collapsed" : ""}`}
			aria-label="Agents"
		>
			<div className="flex items-center justify-between px-6 py-5 border-b border-border">
				{!isCollapsed ? (
					<>
						...
						<div className="flex items-center gap-2">
							...
							{onToggleCollapse && (
								<button
									type="button"
									className="hidden md:inline-flex h-8 w-8 items-center justify-center rounded-lg bg-muted hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
					...
					>
						<span className="text-muted-foreground hover:text-foreground transition-colors text-xl">→</span>
					</button>
				)}
			</div>
			...
			{isCollapsed && (
				<div className="flex-1 overflow-y-auto py-3 flex flex-col items-center gap-4">
					{(agents ?? []).map((agent) => (
						<div
							key={agent._id}
